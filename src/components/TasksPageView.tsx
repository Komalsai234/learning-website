  ...
  return (
    <div className="fixed inset-0 bg-[#f4ede4] z-40 overflow-y-auto">
      <div className="sticky top-0 z-50 bg-gradient-to-r from-[#faf7f2] to-[#f4ede4] border-b border-[#d9cfc1] shadow-lg">
        <div className="px-4 sm:px-6 lg:px-8 py-3 border-b border-[#e5dccf]">
          <div className="max-w-7xl mx-auto">
            <button onClick={onClose}
              ...
              <span className="flex items-center justify-center w-9 h-9 rounded-xl bg-[#e5dccf] group-hover:bg-[#ab6e47] text-[#5c4a3a] group-hover:text-white transition-all duration-200 shadow-sm">
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-4">
          <div className="max-w-7xl mx-auto">
            ...
            {week.description && <p className="mt-2 text-[#8c7a6a] max-w-2xl">{week.description}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-7xl mx-auto">
          {week.tasks.length === 0 ? (
            <div className="text-center py-20 bg-white/40 rounded-2xl border border-[#d9cfc1]/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-24">
              {week.tasks.map((task, taskIndex) => (
                <TaskCard key={taskIndex} task={task} weekIndex={weekIndex} taskIndex={taskIndex}
                  onEdit={onEditTask} onChangeStatus={onChangeStatus} />
              ))}
            </div>
          )}
        </div>
      ...
      {week.tasks.length > 0 && (
        <button onClick={() => onAddTask(weekIndex)}
          className="fixed bottom-8 right-8 w-16 h-16 bg-gradient-to-r from-[#ab6e47] to-[#8b5a3c] text-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 hover:rotate-90 flex items-center justify-center z-50">
