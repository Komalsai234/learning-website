...
import { X, Plus, Trash2, Video, ClipboardList, Loader2 } from 'lucide-react';
...
export function AddTaskModal({ isOpen, onClose, onSave }: AddTaskModalProps) {
  ...
  const footer = (
    <div className="flex gap-3 px-6 py-4" style={{ background: '#f5ede3', borderTop: '1px solid #ddd0bc' }}>
      ...
      <button onClick={handleSave} disabled={saving}
        className="flex-1 inline-flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:shadow-md active:translate-y-0 disabled:opacity-50"
  ...
  const header = (
    <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
      ...
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-['Playfair_Display'] text-lg font-bold text-white">Add New Task</h2>
      </div>
      <button onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg rounded-2xl overflow-hidden animate-scaleIn"
        ...
        {header}
        <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 240px)', background: '#fdfaf6' }}>{formBody}</div>
        {footer}
      </div>
    </div>
  );
