...
function saveNewWeek() {
    ...
    console.log('Week created, total weeks:', weeks.length);
}
...
function saveNewTask() {
    ...
    // Check if tasks page is open and refresh it
    const tasksPage = document.getElementById('tasksPageView');
    if (tasksPage) {
        // Refresh the tasks page view instead of going back
        const weekIndex = currentWeekId;
        tasksPage.remove();
        toggleTasks(weekIndex);
    } else {
        renderWeeks();
    }
